...
const ExcelExportButton = ({ getData, label = "Export to Excel", className = "" }) => {
    ...
    return (
        <button
            ...
            className={`flex items-center gap-2 px-3 sm:px-4 py-2 bg-white border border-gray-200 hover:border-green-500 hover:bg-green-50 text-gray-700 hover:text-green-700 rounded-lg transition-all duration-200 shadow-sm active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed group ${className}`}
        >
            {isExporting ? (
            ...
            {!isExporting && <Download className="h-3.5 w-3.5 ml-1 opacity-0 group-hover:opacity-100 -translate-y-1 group-hover:translate-y-0 transition-all text-green-600" />}
        </button>
    );
};
